...
(function (global) {
  ...
  function waterGlassMarkup({ variant = 'dark', id = '' }) {
    ...
    const uniqueClip = `glassClip${id || Date.now()}`;
    return `
      <div class="water-glass-wrap" data-water-glass${id ? ` id="${id}"` : ''} aria-hidden="true">
        <svg class="water-glass-svg" viewBox="0 0 100 140" xmlns="http://www.w3.org/2000/svg">
          ...
          <g clip-path="url(#${uniqueClip})">
            <rect class="water-fill-rect" data-water-fill x="24" width="52" y="72" height="56" fill="url(#${gradId})"/>
            <ellipse class="water-surface-line" data-water-surface cx="50" cy="72" rx="22" ry="4" fill="rgba(150,220,255,0.8)" opacity="1"/>
          </g>
          <path class="water-pour-stream" d="M46 4 L54 4 L52 42 L48 42 Z"/>
          <circle data-water-ripple cx="50" cy="128" r="2" fill="rgba(255,255,255,0.5)" opacity="0"/>
        </svg>
      </div>`;
  }

  function setGlassLevel(wrap, liters, animatePour, goal) {
    if (!wrap) return;
    const fill = wrap.querySelector('[data-water-fill]');
    const surface = wrap.querySelector('[data-water-surface]');
    const targetGoal = goal || WATER_GOAL_L;
    const pct = Math.min(liters / targetGoal, 1);
    ...
    if (fill) {
      ...
      // Ensure fill is visible with fallback color
      if (!fill.getAttribute('fill') || fill.getAttribute('fill') === 'none') {
        fill.setAttribute('fill', '#1a9fd4');
      }
    }
    if (surface) {
  ...
  function initWaterWidget(root, options) {
    ...
    const update = (pour) => {
      setGlassLevel(wrap, liters, pour, goal);
